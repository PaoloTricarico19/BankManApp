...
public class ContoService {
    ...
    public Conto trovaPerId(int id) {
        log.debug("Ricerca conto (entità completa) con ID {}", id);
        if (id <= 0) {
            throw new IllegalArgumentException("ID non valido: deve essere un valore numerico positivo.");
        }
    ...
    public List<ContoDto> findAll() {
        log.debug("Richiesta lista completa conti");
        List<ContoDto> lista = contoRepository.findAll().stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
        log.info("Trovati {} conti totali", lista.size());
        return lista;
    }
