...
public class MovimentiService {
    ...
    public MovimentiDto update(int id, Movimenti movimentiAggiornato) {
        log.debug("Aggiornamento movimento {} con dati {}", id, movimentiAggiornato);

        Movimenti movimentiEsistente = movimentoRepository.findById(id)
                .orElseThrow(() -> {
                    log.warn("Impossibile aggiornare: movimento {} non trovato", id);
                    return new RuntimeException("Movimento non trovato con ID: " + id);
                });
    ...
    private MovimentiDto convertToDto(Movimenti movimenti) {
        return new MovimentiDto(
                movimenti.getId(),
                movimenti.getConto().getId(),
                movimenti.getCarta() != null ? movimenti.getCarta().getId() : 0,
